...
class Innslag
{
    ...
    **/
    ...
    private function _loadByBID( $b_id, $select_also_if_not_completed ) {
        $SQL = new Query(self::getLoadQuery()."
    ...
    **/
    public function getSubscriptionTime() {
        ...
        if( empty( $this->subscriptionTime ) ) {
            $qry = new Query("SELECT `log_time` FROM `ukmno_smartukm_log`
                            WHERE `log_b_id` = '#bid'
                            AND `log_code` = '22'
