...
class VCard {
  ...
  */
  function vcard($initial_data=false) {
  ...
  function build() {
    ...
    */
    ...
    if ($this->data['url']) { $this->card .= "URL;TYPE=work:".$this->data['url']."\r\n"; }
  	...
  	if ($this->data['photo']) {
  		$fileext = strtoupper( substr($this->data['photo'], strrpos($this->data['photo'],'.')+1 ) );
  		$this->card .= 'PHOTO;VALUE=URL;TYPE='.$fileext.':'.$this->data['photo']."\r\n";
  		$this->card .= 'LOGO;'.$fileext.':'.$this->data['photo']."\r\n";
  	}
  ...
  */
  ...
  function store($int_path, $ext_path) {
  	$fh = fopen($int_path.'.vcf','w');
	fwrite($fh, $this->card);
	fclose($fh);
	return $ext_path.'.vcf';
  }
