...
class write_monstring
{
        ...
        switch ($type) {
            case 'kommune':
                if (!is_array($geografi)) {
                    throw new Exception(
                        'Monstring_V2::create: Geografiobjekt må være array kommuner, ikke' . (is_object($geografi) ? get_class($geografi) : is_array($geografi) ? 'array' : is_integer($geografi) ? 'integer' : is_string($geografi) ? 'string' : 'ukjent datatype')
                    );
                }
    ...
    public static function _leggTilVideresendingAvsender($monstring_save, $avsender)
    {
        ...
        $insert = new SQLins('ukm_rel_pl_videresending');
